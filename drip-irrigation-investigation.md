...
## 如果目标确定为HYDRUS二维湿润体精细对比

这个目标下，当前结论应改为“工具链已具备，模型实现未达标”。也就是说，现有代码已经能做同网格、同土壤参数、同初始条件、同滴灌事件的二维图像对照；但现有主线仍主要依赖`DripSpreadMode=1`宽度标定和`DripMode=3`直接源项分配，不能作为严格的HYDRUS SurfaceDrip物理复现。

因此后续不能再把目标定义为“滴灌水有没有进入模型”或“二维图看起来有响应”，而要定义为：

- 边界条件要接近HYDRUS：滴头节点先施加通量，达到压力头上限后切换为压力边界，并把不能接纳的超额水在同一步内分配到相邻地表边界。
- 水量诊断要闭合：输入滴灌量、实际边界入渗量、土体储水增量、地表滞留/径流、排水和数值残差都要能逐项核对。
- 二维形态要验收：同时比较`Delta theta`场、残差场、湿润锋轮廓、连通湿润体宽度/深度、IoU、峰值位置、储水比例和阈值敏感性。
- 根系只作为作物算例的叠加层：官方HYDRUS Drip1/Drip2是无作物短时算例，不能直接拿来验证根系吸水；若要看滴灌与根系匹配，需要另设带根系分布的MAIZSIM作物算例，并输出二维根系权重图。

按这个目标，推荐把代码路线拆成两层：

- 保留`DripSpreadMode=1`和`DripMode=3`作为工程回归基线，用来确认绘图、网格、体积权重、水量统计和HYDRUS对照工具链没有退化。
- 重做`DripSpreadMode=2`作为真正的压力头受限边界模式。它不应再依赖HYDRUS最终湿润宽度曲线作为输入，而应由`WaterMover`中的实际压力头、实际入渗量和超额水重分配来决定地表活动边界。

当前`DripSpreadMode=2`只能算原型：低流量烟测能跑通，官方`2 L/h` Drip2仍会在滴灌开始后缩到极小时间步并超时。因此若项目目标锁定为HYDRUS二维湿润体精细对比，下一步应优先实现`mode=2`主动集迭代和稳定子步长，而不是继续调`DripWetWidthMax`或直接源项深度。
