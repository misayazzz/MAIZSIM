...
- `6`: HYDRUS-style surface drip approximation. This is surface drip only. Low-flow cases use an active flux/head boundary at the surface; numerically stiff high-flow cases use a localized stable surface-source fallback bounded by `DripWetWidthMax` and are reported with accepted/remaining water in G05.
